    ...
    try
        ...
        try
            if haskey(resultados_espectros, "periodos")
                ...
                if haskey(resultados_espectros, "Sa_rock") && length(resultados_espectros["Sa_rock"]) > 0
                    plot!(p[3], periodos, resultados_espectros["Sa_rock"],
                          label="Roca",
                          color=:red,
                          lw=2)
                end
                
                if haskey(resultados_espectros, "Sa_convolucion") && length(resultados_espectros["Sa_convolucion"]) > 0
                    plot!(p[3], periodos, resultados_espectros["Sa_convolucion"],
                          label="Reconstruido",
                          color=:green,
                          lw=2,
                          linestyle=:dash)
        ...
        try
            if haskey(resultados_movimientos, "gamma_max") && length(subcapas) > 0
                ...
                if length(deformaciones) > 0
                    ...
                    # Configurar límites apropiados para escala logarítmica
                    x_min = 0.001  # Límite mínimo 0.001%
                    x_max = max(def_max*1.5, 1.0)  # Máximo hasta 1% o 1.5 veces el máximo
                    xlims!(p[5], (x_min, x_max))
