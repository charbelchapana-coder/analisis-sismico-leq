...
"""
    calcular_movimientos_y_deformaciones(capas_finales::Vector{capa_suelo}, capas_originales::Vector{capa_suelo},
                                        ...
                                        Amp_final::Vector{ComplexF64}, tiempo::Vector{Float64}, dt::Float64, max_iter_conv::Int=200, tolerancia_conv::Float64=1e-4, 
                                        archivo_deconvolucion::String="deconvolucion_roca.txt")

Función principal para el cálculo de movimientos y deformaciones.
Implementa la Etapa 5 del análisis.

# Parámetros adicionales:
- `max_iter_conv`: Número máximo de iteraciones para la convolución iterativa (por defecto 200)
- `tolerancia_conv`: Tolerancia de convergencia para la convolución iterativa (por defecto 1e-4)
- `archivo_deconvolucion`: Nombre del archivo de salida para la señal deconvolucionada (por defecto "deconvolucion_roca.txt")

# Funcionalidad de guardado:
La función guarda automáticamente la señal sísmica deconvolucionada en roca en un archivo de texto
con el nombre especificado en `archivo_deconvolucion`. El archivo contiene dos columnas separadas por tabs:
- Columna 1: Tiempo (s) con 3 decimales de precisión
- Columna 2: Aceleración (m/s²) con 4 decimales de precisión
"""
function calcular_movimientos_y_deformaciones(capas_finales::Vector{capa_suelo}, capas_originales::Vector{capa_suelo},
    ...
    println("--------------------------------------------------------")
    nombre_archivo = archivo_deconvolucion
