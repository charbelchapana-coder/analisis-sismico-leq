...
using ..Lib_TransferenciaCore: capa_suelo, calcular_matriz_transferencia, parametros_degradacion, deformaciones_corte_shake, funcion_transferencia
...
function calcular_movimientos_y_deformaciones(capas_finales::Vector{capa_suelo}, capas_originales::Vector{capa_suelo},
    ...
    try
        ...
        # Guardar archivo con encabezados
        open(nombre_archivo, "w") do archivo
            # Escribir encabezado
            println(archivo, "# Señal deconvolucionada en roca basal")
            println(archivo, "# Columna 1: Tiempo (s)")
            println(archivo, "# Columna 2: Aceleración (m/s²)")
            println(archivo, "# PGA roca: $(round(maximum(abs.(acc_rock)), digits=4)) m/s²")
            println(archivo, "# Duración: $(round(maximum(tiempo_adj), digits=2)) s")
            println(archivo, "# Puntos: $(length(tiempo_adj))")
            println(archivo, "# Generado: $(now())")
            println(archivo, "#")
            println(archivo, "# Tiempo(s)    Aceleracion(m/s2)")
            
            # Escribir datos con redondeo específico
