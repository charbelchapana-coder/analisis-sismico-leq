...
struct parametros_degradacion
    # Parámetros en orden: (a, γ_ref, b1, b2, b3)
    a::Float64          # Parámetro "a" para curva de degradación G/Go = 1 / (1 + (γ / γ_ref)^a)
    γ_ref::Float64      # Deformación de referencia γ_ref (sin unidades, sin transformar a %)
    b1::Float64         # Coeficiente b1 para curva cuadrática de amortiguamiento ζ = b1*x² + b2*x + b3
    b2::Float64         # Coeficiente b2 para curva cuadrática de amortiguamiento
    b3::Float64         # Coeficiente b3 para curva cuadrática de amortiguamiento
...
Fórmula: G/Go = 1 / (1 + (γ / γ_ref)^a)
Donde γ está en las mismas unidades que γ_ref (sin transformar a %).
"""
function curva_degradacion(γ, params::parametros_degradacion)
    if params.tipo == "lineal"
        ...
        # Curva de degradación: G/Go = 1 / (1 + (γ / γ_ref)^a)
        return 1.0 / (1 + (γ / params.γ_ref)^params.a)
    end
end
...
Para suelo diatomáceo: Curva cuadrática ζ = b1*x² + b2*x + b3
Donde x = γ * 100 (deformación de corte en porcentaje).
Se aplica limitación física: ζ >= 0.001 (0.1% mínimo)
"""
function curva_amortiguamiento(γ, params::parametros_degradacion)
    if params.tipo == "lineal"
        return params.b3  # Valor constante (b3) para comportamiento lineal
    else
        # Curva cuadrática de amortiguamiento: ζ = b1*(γ*100)² + b2*(γ*100) + b3
        # Convertir γ (decimal) a porcentaje para la función cuadrática
        γ_pct = γ * 100.0
        ζ = params.b1 * γ_pct^2 + params.b2 * γ_pct + params.b3
        
        # Limitación física: amortiguamiento mínimo de 0.1%
        return max(ζ, 0.001)
    end
end
